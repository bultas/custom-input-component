export class CustomInput extends HTMLElement {
  ...
  constructor() {
    ...
    this.shadowRoot.appendChild(this.input); // keep reference to <form> for cleanup
  ...
  findContainingForm() {
    ...
    const forms = Array.from(root.querySelectorAll('form')); // we can only be in one <form>, so the first one to contain us is the correct one
